...
feature:
1. 飞书 Bot 集成 — 可与 Telegram Bot 同时运行
2. 飞书适配器使用 WebSocket 长连接（WSClient）— 无需公网地址
3. 飞书互动卡片按钮支持机器/项目/会话选择
4. 通过飞书 `im.message.update` API 实现流式文本编辑
5. 飞书支持图片和文件上传
6. 多平台 Bridge — 两个 Bot 共享一个 HTTP 服务器和进程

bugfix:
1. 修复飞书消息流式输出不更新（使用了 `patch` 而非 `update` API）
2. 修复 Bridge 会话恢复跳过不同平台的用户
3. 移除卡片操作事件的重复 WebSocket 响应
