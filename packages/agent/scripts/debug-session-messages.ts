 ...
 *
 * 使用: pnpm exec ts-node scripts/debug-session-messages.ts
 * 或在 packages/agent 下: npx ts-node scripts/debug-session-messages.ts
 */
...
function extractFromSdkMessage(msg: { type: string; message?: { content?: unknown } }) {
...
function sdkStats(messages: { type: string; message?: { content?: unknown } }[]) {
  const allToolUse = new Set<string>();
  const allToolResult = new Set<string>();
...
function fileStats(jsonlPath: string) {
  const allToolUse = new Set<string>();
  const allToolResult = new Set<string>();
  ...
  for (const line of raw.split('\n')) {
    ...
    if (!trimmed) continue;
    try {
      const row = JSON.parse(trimmed) as { type?: string; message?: { content?: unknown } };
